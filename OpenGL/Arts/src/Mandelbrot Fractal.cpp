...
const float MAX_INTER = 128.0f;

float mandlebrot(vec2 uv){
  vec2 C =  5.0f * uv - vec2(0.7,0.0);
  vec2 Z = vec2(0.0f);
  float inter = 0.0f;

  for(float i;i < MAX_INTER;i++){
    Z = vec2(pow(Z.x,2) - pow(Z.y,2),2.0f * Z.x * Z.y) + C;
    if(dot(Z,Z) > 4.0f) return inter / MAX_INTER;
    inter++;
  }
  return 0.0f;
}
...
void main()
{
  ...
  vec2 uv = (gl_FragCoord.xy - 0.5 * resolution) / resolution.y; // normalized in range [-1,1]

  vec3 finalColor = vec3(0.0); 

  finalColor.xz += mandlebrot(uv);

  finalColor.y += mandlebrot(uv);
  finalColor.z += mandlebrot(uv);
...
int main()
{
  ...
  GLFWwindow* window = glfwCreateWindow(W,H, "[Mandelbrot Fractal]",NULL,NULL);
