...
void main()
{
  ...
  vec2 position =  gl_FragCoord.xy / resolution.xy; //normalized

  vec2 stepPos = step(vec2(0.2),position);
  
  vec3 color = vec3(stepPos,0.5);
...
int main()
{
  ...
  GLFWwindow* window = glfwCreateWindow(1000,1000, "[GYPB] - [Step function] ",NULL,NULL);
