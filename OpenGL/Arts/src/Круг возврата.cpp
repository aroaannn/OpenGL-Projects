...
void main()
{
  ...
  vec3 finalColor = vec3(0.00); // a black color

  float d = sqrt(pow(uv.x,2) + pow(uv.y,2)); // Find the diagonal of a 1000x1000 normalized in range [-1,1]

  d = fract(2 / d); // fract(x) = floor(z), where z is the fractional part of x.
...
int main()
{
    if(!glfwInit())
        return -1;
    GLFWwindow* window = glfwCreateWindow(1000,1000,"[Круг возврата]",NULL,NULL);
